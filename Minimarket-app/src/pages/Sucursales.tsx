...
import { Building2, Plus, Search, MapPin, Key, RefreshCw, Pencil, Copy } from 'lucide-react';
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/50 dark:bg-gray-900/50">
                ...
                <th className="px-6 py-4 text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest">Dirección</th>
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
              {loading ? (
                ...
                <tr>
                  <td colSpan={4} className="px-6 py-12 text-center text-gray-400 italic">No hay sucursales registradas.</td>
                </tr>
              ) : (
                filteredSedes.map((sede) => (
                  <tr key={sede.id} className="group hover:bg-gray-50/50 dark:hover:bg-gray-900/20 transition-colors">
                    ...
                    <td className="px-6 py-4 text-center">
                      <Badge 
                        label={sede.estado === 'activo' ? 'EN LÍNEA' : 'INACTIVO'} 
                        variant={sede.estado === 'activo' ? 'emerald' : 'gray'} 
                      />
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-2 transition-opacity">
                        {/* Botón Copiar con Tooltip */}
                        <div className="relative group/tooltip">
                          ...
                          <div className="absolute bottom-full right-0 mb-2 w-48 p-2 bg-gray-900 text-white text-[10px] leading-tight rounded-lg opacity-0 invisible group-hover/tooltip:opacity-100 group-hover/tooltip:visible transition-all z-20 shadow-xl pointer-events-none">
                            Copiar nombre y llave de acceso para configurar sede secundaria
