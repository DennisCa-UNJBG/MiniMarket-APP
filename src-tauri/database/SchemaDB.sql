...
CREATE TABLE IF NOT EXISTS unidades_medida (
    ...
    estado      TEXT    DEFAULT 'activo'
                        CHECK (estado IN ('activo', 'inactivo')),  -- [FIX-09]
...
CREATE TABLE IF NOT EXISTS configuracion (
    id               INTEGER PRIMARY KEY AUTOINCREMENT,
    singleton        INTEGER DEFAULT 1 UNIQUE
                             CHECK (singleton = 1),          -- [FIX-12] una sola fila
...
CREATE TABLE IF NOT EXISTS compras_ingresos (
    ...
    estado               TEXT    DEFAULT 'completado'
                                 CHECK (estado IN ('completado', 'anulado')),  -- [FIX-09]
    metodo_pago          TEXT    DEFAULT 'BANCO'
                                 CHECK (metodo_pago IN ('EFECTIVO', 'BANCO', 'TARJETA', 'TRANSFERENCIA')), -- [FIX-09]
...
CREATE TABLE IF NOT EXISTS ventas (
    ...
    metodo_pago    TEXT    DEFAULT 'EFECTIVO'
                           CHECK (metodo_pago IN ('EFECTIVO', 'TARJETA', 'TRANSFERENCIA')), -- [FIX-09]
    ...
    igv_porcentaje REAL    DEFAULT 0 CHECK (igv_porcentaje >= 0 AND igv_porcentaje <= 100),
    estado         TEXT    DEFAULT 'completado'
                           CHECK (estado IN ('completado', 'anulado')),  -- [FIX-09]
    cliente_id     INTEGER,
    sucursal_local_id INTEGER,  -- [IDEM] ID local de la sucursal para detectar duplicados
...
CREATE TABLE IF NOT EXISTS kardex (
    ...
    tipo_movimiento TEXT    NOT NULL
                           CHECK (tipo_movimiento IN ('INGRESO', 'SALIDA', 'AJUSTE')), -- [FIX-09]
    cantidad        REAL   NOT NULL CHECK (cantidad > 0),             -- [FIX-09]
...
CREATE TABLE IF NOT EXISTS cajas (
    ...
    estado          TEXT    DEFAULT 'abierta'
                            CHECK (estado IN ('abierta', 'cerrada')), -- [FIX-09]
    sucursal_id     TEXT,
    sucursal_local_id INTEGER,  -- [IDEM] ID local de la sucursal para detectar duplicados
    sincronizado    INTEGER DEFAULT 0,
    created_at      TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE INDEX IF NOT EXISTS idx_kardex_referencia      ON kardex (referencia);          -- [FIX-11] anulaciones
CREATE INDEX IF NOT EXISTS idx_kardex_sincronizado    ON kardex (sincronizado);        -- [FIX-11] push pendiente

-- Ventas
CREATE INDEX IF NOT EXISTS idx_ventas_fecha           ON ventas (fecha);
CREATE INDEX IF NOT EXISTS idx_ventas_cliente         ON ventas (cliente_id);
CREATE INDEX IF NOT EXISTS idx_ventas_sucursal_fecha  ON ventas (sucursal_id, fecha);  -- [FIX-11] multi-sede
CREATE INDEX IF NOT EXISTS idx_ventas_sincronizado    ON ventas (sincronizado);        -- [FIX-11] push pendiente
...
CREATE INDEX IF NOT EXISTS idx_compras_sucursal_fecha ON compras_ingresos (sucursal_id, fecha); -- [FIX-11]
CREATE INDEX IF NOT EXISTS idx_compras_sincronizado   ON compras_ingresos (sincronizado);       -- [FIX-11]

-- Productos
CREATE INDEX IF NOT EXISTS idx_productos_categoria    ON productos (categoria_id);     -- [FIX-11]
CREATE INDEX IF NOT EXISTS idx_productos_sincronizado ON productos (sincronizado);     -- [FIX-11]

-- Precios (consulta frecuente: precio actual de un producto)
CREATE INDEX IF NOT EXISTS idx_precios_producto_activo ON precios_historial (producto_id, activo); -- [FIX-11]

-- Logs
CREATE INDEX IF NOT EXISTS idx_logs_usuario           ON logs (usuario_id);
CREATE INDEX IF NOT EXISTS idx_logs_fecha             ON logs (created_at);            -- [FIX-10] columna correcta
CREATE INDEX IF NOT EXISTS idx_logs_sincronizado      ON logs (sincronizado);          -- [FIX-11]
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_ventas_origen
    ON ventas (sucursal_id, sucursal_local_id)
    WHERE sucursal_id IS NOT NULL AND sucursal_local_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS idx_compras_origen
    ON compras_ingresos (sucursal_id, sucursal_local_id)
    WHERE sucursal_id IS NOT NULL AND sucursal_local_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS idx_kardex_origen
    ON kardex (sucursal_id, sucursal_local_id)
    WHERE sucursal_id IS NOT NULL AND sucursal_local_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS idx_cajas_origen
    ON cajas (sucursal_id, sucursal_local_id)
    WHERE sucursal_id IS NOT NULL AND sucursal_local_id IS NOT NULL;
...
INSERT OR IGNORE INTO roles (id, nombre, descripcion, permisos)
VALUES
    (1, 'Administrador', 'Acceso total al sistema',             '["*"]'),
    (2, 'Cajero',        'Realiza ventas y visualiza productos','["pos", "ventas", "productos", "clientes", "reportes"]'),
    (3, 'Almacén',       'Gestiona inventario, compras y kardex','["inventario", "productos", "compras", "kardex", "reportes"]');

-- Usuario administrador inicial — contraseña: admin
INSERT OR IGNORE INTO usuarios (id, username, password_hash, nombre_completo, rol_id, estado)
VALUES (
    1, 'admin',
    '$2b$10$lA3xmVfn7iicVDiAf.nQU.UdPoeQBG6bvS0Kv2DJm9QM7l6tYpthG',
    'Administrador del Sistema', 1, 'activo'
);

INSERT OR IGNORE INTO categorias (nombre, color)
VALUES
    ('Abarrotes', '#f59e0b'),
    ('Bebidas',   '#0ea5e9'),
    ('Lácteos',   '#60a5fa'),
...
INSERT OR IGNORE INTO unidades_medida (nombre, abreviatura)
VALUES
    ('Unidad',    'UND'),
    ('Kilogramo', 'KG'),
    ('Litro',     'LT'),
